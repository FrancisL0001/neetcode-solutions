...
class Solution:
    def maxPathSum(self, root: Optional[TreeNode]) -> int:
        ...
        def dfs(node):
            ...
            runningValidMax = max(node.val, node.val + left, node.val + right)
            # the running valid max is basically considering only one or the 
            # other side of the tree to be passes up the tree for calculating paths

            res = max(res, curMax)

            return runningValidMax
